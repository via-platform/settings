    ...
    return (
      <div className='error-message'>
        <div ref='alert' className='alert alert-danger alert-dismissable native-key-bindings' tabIndex='-1'>
          <button ref='close' className='close icon icon-x' onclick={() => this.destroy()}></button>
          <span ref='message' className='native-key-bindings'></span>
          ...
          <div ref='detailsArea' className='padded'>
            <pre ref='details' className='error-details text'></pre>
          </div>
        </div>
      </div>
    )
  ...
  render () {
    return (
      <div>
        ...
        <div className='compile-tools-message'>
          <span>Run </span>
          <code className='alert-danger'>vpm install --check</code>
