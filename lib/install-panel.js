...
import {CompositeDisposable, TextEditor} from 'via'
...
export default class InstallPanel {
  constructor (settingsView, packageManager) {
    ...
    this.viaWorldURL = 'https://via.world/packages'
  ...
  render () {
    return (
      <div className='panels-item install-panel' tabIndex='-1'>
        <div className='section packages'>
          <div className='section-container packages-container'>
            <h1 ref='installHeading' className='section-heading'>Install Packages</h1>
            ...
            <div className='text native-key-bindings' tabIndex='-1'>
              ...
              <a className='link' onclick={this.didClickOpenViaWorld.bind(this)}>via.world</a>
              <span> and are installed to {path.join(process.env.VIA_HOME, 'packages')}</span>
            </div>

            <form className='search-container native-key-bindings' onSubmit={this.performSearch.bind(this)}>
                <input type='search' ref='search' className='input-text' placeholder='Search Packages...' />
            </form>
        ...
        <div className='section packages'>
          <div className='section-container'>
            <div ref='featuredHeading' className='section-heading' />
            <div ref='featuredErrors' />
            <div ref='loadingMessage' className='alert alert-info' />
  ...
  setSearchType (searchType) {
    if (searchType === 'theme') {
      ...
      // this.refs.search.setPlaceholderText('Search themes')
      this.refs.publishedToText.textContent = 'Themes are published to '
      this.viaWorldURL = 'https://via.world/themes'
      ...
      // this.refs.search.setPlaceholderText('Search packages')
  ...
  beforeShow (options) {
    if (options && options.uri) {
      ...
      if (query != null) {
        ...
        this.refs.search.value = packageName
        this.performSearch()
      }
    }
  }
  ...
  didClickOpenViaWorld (event) {
    event.preventDefault()
    electron.shell.openExternal(this.viaWorldURL)
  }
