...
export default class KeybindingsPanel {
  constructor () {
    ...
    this.disposables.add(via.keymaps.onDidReloadKeymap(() => { this.loadKeyBindings() }))
    this.disposables.add(via.keymaps.onDidUnloadKeymap(() => { this.loadKeyBindings() }))
      ...
      <div className='panels-item' tabIndex='-1'>
        <section className='keybinding-panel section'>
          ...
          <div className='editor-container'>
            <TextEditor mini={true} ref='searchEditor' placeholderText='Search keybindings' />
          </div>
  ...
  loadKeyBindings () {
    ...
    this.keyBindings = _.sortBy(via.keymaps.getKeyBindings(), 'keystrokes')
...
`
      ...
      return via.clipboard.write(content)
    }
  ...
  didClickOpenKeymapFile (e) {
    e.preventDefault()
    via.commands.dispatch(via.views.getView(via.workspace), 'application:open-your-keymap')
  }
  ...
  static determineSource(filePath) {
    ...
    if (filePath.indexOf(path.join(via.getLoadSettings().resourcePath, 'keymaps')) === 0) {
      return 'Core'
    } else if (filePath === via.keymaps.getUserKeymapPath()) {
      return 'User'
    } else {
