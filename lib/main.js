...
const SnippetsProvider = {
  getSnippets() { return via.config.scopedSettingsStore.propertySets }
};
...
class SettingsPackage {
    activate(){
        ...
        via.commands.add('via-workspace', {
            ...
            // 'settings:editor': () => via.workspace.open(`${BaseURI}/editor`),
            // 'settings:show-keybindings': () => via.workspace.open(`${BaseURI}/keybindings`),
            // 'settings:change-themes': () => via.workspace.open(`${BaseURI}/themes`),
            'settings:install-packages-and-themes': () => via.workspace.open(`${BaseURI}/install`),
            // 'settings:view-installed-themes': () => via.workspace.open(`${BaseURI}/themes`),
            // 'settings:uninstall-themes': () => via.workspace.open(`${BaseURI}/themes`),
            'settings:view-installed-packages': () => via.workspace.open(`${BaseURI}/packages`),
            'settings:uninstall-packages': () => via.workspace.open(`${BaseURI}/packages`),
            'settings:check-for-package-updates': () => via.workspace.open(`${BaseURI}/updates`)
        });
        ...
        const notification = via.notifications.addWarning(
            ...
            `${deprecatedPackages.length} ${packageText} ${have} deprecations and ${were} not loaded.`,
            {
                ...
                buttons: [{
                    text: 'View Packages',
                    onDidClick: () => {
                        via.commands.dispatch(via.views.getView(via.workspace), 'settings-view:view-installed-packages');
