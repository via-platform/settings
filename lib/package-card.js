...
export default class PackageCard {
  constructor (pack, settingsView, packageManager, options = {}) {
    ...
    if (this.pack.vpmInstallSource && this.pack.vpmInstallSource.type === 'git') {
      if (this.pack.vpmInstallSource.sha !== this.pack.latestSha) {
    ...
    if (via.packages.isBundledPackage(this.pack.name)) {
        ...
        <div className='package-card'>
            <div className='body'>
                <div className='package-title'>
                    <h4 className='package-name' ref='packageName'>{displayName}</h4>
                    <div className='package-version' ref='versionValue'>{String(this.pack.version)}</div>
                    <div className='deprecation-badge highlight-warning'>Deprecated</div>
                </div>
                <div ref='packageDescription' className='package-description'>{description}</div>
                <div ref='packageMessage' className='package-message' />
            </div>
            <div ref='updateButtonGroup' className='btn-group'>
                <button type='button' className='btn btn-primary install-button' ref='updateButton'>Update</button>
            </div>
            <div ref='installAlternativeButtonGroup' className='btn-group'>
                <button type='button' className='btn btn-primary install-button' ref='installAlternativeButton'>Install Alternative</button>
            </div>
            <div ref='installButtonGroup' className='btn-group'>
                <button type='button' className='btn btn-primary install-button' ref='installButton'>Install Package</button>
            </div>
            <div ref='packageActionButtonGroup' className='btn-group'>
                <button type='button' className='btn settings' ref='settingsButton'>Settings</button>
                <button type='button' className='btn uninstall-button' ref='uninstallButton'>Uninstall</button>
                <button type='button' className='btn enablement' ref='enablementButton'>
    ...
    this.packageManager.loadCompatiblePackageVersion(this.pack.name, (err, pack) => {
      ...
      if (packageVersion) {
        ...
        if (packageVersion !== this.pack.version) {
          ...
          this.refs.packageMessage.textContent = `Version ${packageVersion} is not the latest version available for this package, but it's the latest that is compatible with your version of Via.`
        }
        ...
        this.refs.packageMessage.insertAdjacentText(
          'beforeend',
          `There's no version of this package that is compatible with your Via version. The version must satisfy ${this.pack.engines.via}.`
        )
        console.error(`No available version compatible with the installed Via version: ${via.getVersion()}`)
      }
  ...
  handleButtonEvents (options) {
    ...
    const updateButtonClickHandler = (event) => {
      ...
      this.update().then(() => {
        const buttons = [{
          text: 'Restart',
          onDidClick () { return via.restartApplication() }
        }]

        via.notifications.addSuccess(`Restart Via to complete the update of \`${this.pack.name}\`.`, {
    ...
    const packageNameClickHandler = (event) => {
      ...
      shell.openExternal(`https://via.world/${packageType}/${this.pack.name}`)
    }
    ...
    const enablementButtonClickHandler = (event) => {
      ...
      if (this.isDisabled()) {
        via.packages.enablePackage(this.pack.name)
      } else {
        via.packages.disablePackage(this.pack.name)
      }
    }
    ...
    const packageMessageClickHandler = (event) => {
      ...
      if (target) {
        event.stopPropagation()
        event.preventDefault()
        if (target.href && target.href.startsWith('via:')) {
          via.workspace.open(target.href)
        }
      }
    }
  ...
  displayNotInstalledState () {
    ...
    const viaVersion = this.packageManager.normalizeVersion(via.getVersion())
    if (!this.packageManager.satisfiesVersion(viaVersion, this.pack)) {
      this.hasCompatibleVersion = false
  ...
  displayDeprecatedState () {
    ...
    if (info && info.hasDeprecations) {
      ...
      if (isInstalled && this.packageManager.isPackageInstalled(alt)) {
        ...
        message = `\`${this.pack.name}\` has been replaced by [\`${alt}\`](via://settings/install/package:${alt}).`
  ...
  handlePackageEvents () {
    let version
    this.disposables.add(via.packages.onDidDeactivatePackage((pack) => {
      if (pack.name === this.pack.name) {
        this.updateDisabledState()
      }
    }))

    this.disposables.add(via.packages.onDidActivatePackage((pack) => {
      if (pack.name === this.pack.name) {
        this.updateDisabledState()
      }
    }))

    this.disposables.add(via.config.onDidChange('core.disabledPackages', () => {
      this.updateDisabledState()
    }))
    ...
    this.subscribeToPackageEvent('package-installed package-install-failed theme-installed theme-install-failed', () => {
      const loadedPack = via.packages.getLoadedPackage(this.pack.name)
    ...
    this.subscribeToPackageEvent('package-updated theme-updated', () => {
      ...
      if (metadata && metadata.vpmInstallSource) {
        this.pack.vpmInstallSource = metadata.vpmInstallSource
      }
  ...
  isDisabled () {
    return via.packages.isPackageDisabled(this.pack.name)
  }

  isDeprecated (version) {
    const packVersion = version != null ? version : this.pack.version
    return via.packages.isDeprecatedPackage(this.pack.name, packVersion)
  }

  getDeprecatedPackageMetadata () {
    return via.packages.getDeprecatedPackageMetadata(this.pack.name)
  }
  ...
  */
  ...
  update () {
    ...
    return new Promise((resolve, reject) => {
      this.packageManager.update(pack, this.newVersion, error => {
        if (error != null) {
          via.assert(false, 'Package update failed', assertionError => {
  ...
  installAlternative () {
    ...
    const loadedPack = via.packages.getLoadedPackage(metadata ? metadata.alternative : null)
