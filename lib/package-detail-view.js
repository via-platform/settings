...
export default class PackageDetailView {
  constructor (pack, settingsView, packageManager) {
    this.pack = pack
    ...
    const openButtonClickHandler = (event) => {
      ...
      if (fs.existsSync(this.pack.path)) {
        via.open({pathsToOpen: [this.pack.path]})
      }
    }
    ...
    const learnMoreButtonClickHandler = (event) => {
      event.preventDefault()
      shell.openExternal(`https://via.world/packages/${this.pack.name}`)
    }
  ...
  loadPackage () {
    const loadedPackage = via.packages.getLoadedPackage(this.pack.name)
    if (loadedPackage) {
  ...
  activateConfig () {
    ...
    if (via.packages.isPackageLoaded(this.pack.name) && !via.packages.isPackageActive(this.pack.name)) {
        ...
        <section className='section'>
          <form className='section-container package-detail-view'>
            ...
            <div ref='buttons' className='btn-wrap-group hidden'>
              <button ref='learnMoreButton' className='btn btn-default icon icon-link'>View on via.world</button>
  ...
  updateInstalledState () {
    ...
    if (via.packages.isPackageLoaded(this.pack.name)) {
      if (!via.packages.isPackageDisabled(this.pack.name)) {
        ...
        //TODO Format the HTML time that is printed on the line below
    ...
    const sourceIsAvailable = this.packageManager.isPackageInstalled(this.pack.name) && !via.packages.isBundledPackage(this.pack.name)
  ...
  openMarkdownFile (path) {
    if (via.packages.isPackageActive('markdown-preview')) {
      via.workspace.open(encodeURI(`markdown-preview://${path}`))
    } else {
      via.workspace.open(path)
    }
  }
  ...
  updateFileButtons () {
    ...
    const packagePath = this.pack.path != null ? this.pack.path : via.packages.resolvePackagePath(this.pack.name)
