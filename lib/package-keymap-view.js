...
import {Disposable, CompositeDisposable} from 'via'
...
export default class PackageKeymapView {
  constructor (pack) {
    ...
    const packagesWithKeymapsDisabled = via.config.get('core.packagesWithKeymapsDisabled') || []
    ...
    const changeHandler = (event) => {
      ...
      if (value) {
        via.config.removeAtKeyPath('core.packagesWithKeymapsDisabled', this.namespace)
      } else {
        via.config.pushAtKeyPath('core.packagesWithKeymapsDisabled', this.namespace)
      }
    ...
    for (let [packageKeymapsPath, map] of via.packages.getLoadedPackage(this.namespace).keymaps) {
      if (map.length > 0) {
        hasKeymaps = true
        break
      }
    ...
    return (
      <section className='section'>
        ...
        <table className='package-keymap-table table native-key-bindings text' tabIndex='-1'>
          ...
          <tbody ref='keybindingItems'></tbody>
        </table>
      </section>
    )
  }
  ...
  updateKeyBindingView () {
    ...
    const keyBindings = []
    if (via.keymaps.build) {
      for (const [keymapPath, keymap] of via.packages.getLoadedPackage(this.namespace).keymaps) {
        keyBindings.push(...via.keymaps.build(this.namespace, keymap, 0, false))
      }
    } else {
      // Backwards compatibility for via <= 1.19
      for (const keyBinding of via.keymaps.getKeyBindings()) {
  ...
  writeKeyBindingToClipboard ({selector, keystrokes, command}) {
    let content
    const keymapExtension = path.extname(via.keymaps.getUserKeymapPath())
