...
export default class SettingsPanel extends CollapsibleSectionPanel {
    constructor(options = {}) {
        ...
        if (this.options.scopeName) {
            ...
            for (const name of SCOPED_SETTINGS) {
                settings[name] = via.config.get(name, {
                    scope: [this.options.scopeName]
                })
            }
        } else {
            settings = via.config.get(namespace)
        }

        if(this.options.values){
            Object.assign(this.options.values, settings);
        }
    ...
    elementForSettings(namespace, settings) {
        ...
        heading.classList.add('block', 'section-heading')
    ...
    bindInputFields() {
        const disposables = Array.from(this.element.querySelectorAll('input[id]')).map((input) => {
            ...
            if(input.type === 'text' && defaultValue){
                input.setAttribute('placeholder', `Default: ${defaultValue}`);
            }
    ...
    observe(name, callback) {
        let params = {
            sources: [via.config.getUserConfigPath()]
        }
        if (this.options.scopeName != null) {
            params.scope = [this.options.scopeName]
        }
        this.disposables.add(via.config.observe(name, params, callback))
    }
    ...
    isDefault(name) {
        ...
        let value = via.config.get(name, params)
        return (value == null) || (defaultValue === value)
    }

    getDefault(name) {
        let params = {
            excludeSources: [via.config.getUserConfigPath()]
        }
        if (this.options.scopeName != null) {
            params.scope = [this.options.scopeName]
        }

        let defaultValue = via.config.get(name, params)
        if (this.options.scopeName != null) {
            ...
            if (via.config.get(name, {
                    excludeSources: [via.config.getUserConfigPath()]
                }) === defaultValue) {
                defaultValue = via.config.get(name)
            }
        }
        return defaultValue
    }
    ...
    set(name, value) {
        ...
        }else if (this.options.scopeName) {
            if (value === undefined) {
                via.config.unset(name, {
                    scopeSelector: this.options.scopeName
                })
            } else {
                via.config.set(name, value, {
                    scopeSelector: this.options.scopeName
                })
            }
        } else {
    ...
    bindTooltips() {
        const disposables = Array.from(this.element.querySelectorAll('input[id], select[id], via-text-editor[id]')).map((element) => {
            let defaultValue = this.valueToString(this.getDefault(element.id))
            if (defaultValue != null) {
                return via.tooltips.add(element, {
...
function sortSettings(namespace, settings) {
    return _.chain(settings)
        ...
        .sortBy((name) => name)
        .sortBy((name) => {
            const schema = via.config.getSchema(`${namespace}.${name}`)
            return schema ? schema.order : null
        })
        .value()
}

function elementForSetting(namespace, name, value, defaultValue) {
    if (namespace === 'core') {
        if (name === 'themes') {
            return document.createDocumentFragment()
    ...
    let schema = via.config.getSchema(`${namespace}.${name}`)
    if (schema && schema.enum) {
        ...
        if (isEditableArray(value)) {
            controls.appendChild(elementForArray(namespace, name, value, defaultValue))
        }
    } else if (_.isObject(value) || (schema && schema.type === 'object')) {
        controls.appendChild(elementForObject(namespace, name, value))
    } else {
        controls.appendChild(elementForString(namespace, name, value, defaultValue))
    }
...
function getSettingTitle(keyPath, name) {
    ...
    const schema = via.config.getSchema(keyPath)
...
function elementForString(namespace, name, value) {
    let keyPath = `${namespace}.${name}`
    let type = _.isNumber(value) ? 'number' : 'text';
