...
module.exports = class UpdatesPanel {
    constructor(settingsView, packageManager){
        ...
        this.disposables.add(
            this.packageManager.on('package-updated theme-updated package-update-failed theme-update-failed', ({
                pack, error
            }) => {
                if (error != null){
    ...
    render(){
        return ( < div tabIndex = '0'
            ...
            < h1 className = 'section-heading icon icon-cloud-download' > Available Updates < /h1> < div className = 'section-heading updates-btn-group' >
            ...
            for Updates < /button> < button ref = 'updateAllButton'
            className = 'update-all-button btn btn-primary'
            onclick = {
                () => {
                    this.updateAll()
                }
            } > Update All < /button> < /div> < /div>

            < div ref = 'versionPinnedPackagesMessage'
            className = 'alert alert-warning icon icon-alert' > The following packages are pinned to their current version and are not being checked
            for updates: < strong > {
                this.packageManager.getVersionPinnedPackages().join(', ')
            } < /strong></div >
            < div ref = 'updateErrors' > < /div> < div ref = 'checkingMessage'
            className = 'alert alert-info icon icon-hourglass' > {
                `Checking for updates\u2026`
            } < /div> < div ref = 'noUpdatesMessage'
            className = 'alert alert-info icon icon-heart' > All of your installed packages are up to date! < /div> < div ref = 'updatesContainer'
            className = 'container package-container' > < /div> < /div> < /section> < /div>
        )
    }
    ...
    addUpdateViews(){
        ...
        for (const pack of this.availableUpdates){
            const packageCard = new PackageCard(pack, this.settingsView, this.packageManager, {
                back: 'Updates'
            })
