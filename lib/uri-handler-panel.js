...
function isDefaultProtocolClient () {
  return require('electron').remote.app.isDefaultProtocolClient('via', process.execPath, ['--uri-handler'])
}
...
function setAsDefaultProtocolClient () {
  ...
  return isSupported() && require('electron').remote.app.setAsDefaultProtocolClient('via', process.execPath, ['--uri-handler'])
}
...
export default class UriHandlerPanel {
  constructor () {
    ...
    this.subscriptions.add(
      ...
      via.uriHandlerRegistry.onHistoryChange(() => {
        this.uriHistory = via.uriHandlerRegistry.getRecentlyHandledURIs()
        etch.update(this)
      })
    )
  }
  ...
  render () {
    const schema = via.config.getSchema('core.uriHandlerRegistration')
        ...
        <form className='general-panel section'>
          <div className='settings-panel'>
            <div className='section-container'>
              ...
              <div className='text icon icon-question'>These settings determine how Via handles via:// URIs.</div>
              <div className='section-body'>
                <div className='control-group'>
                  <div className='controls'>
                    ...
                    >
                      Register as default via:// protocol handler
                    </button>
                  </div>
                </div>
                ...
                <div className='control-group'>
                  <div className='controls'>
                    ...
                    <select
                      ...
                      value={via.config.get('core.uriHandlerRegistration')}
                    >
  ...
  renderItem (item) {
    if (item.host === 'core') {
      ...
      return <a href={`via://settings/packages/${item.host}`} onClick={this.handlePackageLinkClicked}>{item.host}</a>
    }
  }

  handlePackageLinkClicked (evt) {
    evt.preventDefault()
    via.workspace.open(evt.target.getAttribute('href'))
  }

  renderRegistrationDescription () {
    if (this.isDefaultProtocolClient) {
      return 'Via is already the default handler for via:// URIs.'
    } else if (isSupported()) {
      return 'Register Via as the default handler for via:// URIs.'
    } else {
      return 'Registration as the default handler for via:// URIs is only supported on Windows and macOS.'
    }
  }

  handleChange (evt) {
    via.config.set('core.uriHandlerRegistration', evt.target.value)
  }
  ...
  handleBecomeProtocolClient (evt) {
    ...
    if (setAsDefaultProtocolClient()) {
      ...
      via.notifications.addError('Could not become default protocol client')
    }
  }
